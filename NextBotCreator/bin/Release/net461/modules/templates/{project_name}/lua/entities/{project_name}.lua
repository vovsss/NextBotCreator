...
ENT.JumpSound = Sound("{project_name}/jump.mp3")
ENT.JumpHighSound = Sound("{project_name}/spring.mp3")
ENT.TauntSounds = {
	Sound("{project_name}/pieceofcake.mp3"),
	Sound("{project_name}/stepitup.mp3"),
	Sound("{project_name}/tooeasy.mp3"),
	Sound("{project_name}/tooslow.mp3"),
}
local chaseMusic = Sound("{project_name}/panic.mp3")
...
local {project_name}_acquire_distance =
	CreateConVar("{project_name}_acquire_distance", 2500, FCVAR_NONE,
	"The maximum distance at which {project_name} will chase a target.")

local {project_name}_spawn_protect =
	CreateConVar("{project_name}_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, {project_name} will not target players or hide within 200 units of \z
	a spawn point.")

local {project_name}_attack_distance =
	CreateConVar("{project_name}_attack_distance", 80, FCVAR_NONE,
	"The reach of {project_name}'s attack.")

local {project_name}_attack_interval =
	CreateConVar("{project_name}_attack_interval", 0.2, FCVAR_NONE,
	"The delay between {project_name}'s attacks.")

local {project_name}_attack_force =
	CreateConVar("{project_name}_attack_force", 800, FCVAR_NONE,
	"The physical force of {project_name}'s attack. Higher values throw things \z
	farther.")

local {project_name}_smash_props =
	CreateConVar("{project_name}_smash_props", 1, FCVAR_NONE,
	"If set to 1, {project_name} will punch through any props placed in their way.")

local {project_name}_allow_jump =
	CreateConVar("{project_name}_allow_jump", 1, FCVAR_NONE,
	"If set to 1, {project_name} will be able to jump.")

local {project_name}_hiding_scan_interval =
	CreateConVar("{project_name}_hiding_scan_interval", 3, FCVAR_NONE,
	"{project_name} will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant {project_name}s are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local {project_name}_hiding_repath_interval =
	CreateConVar("{project_name}_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to {project_name}'s hiding spot will be redetermined every X seconds.")

local {project_name}_chase_repath_interval =
	CreateConVar("{project_name}_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of {project_name}'s target will be redetermined every \z
	X seconds.")

local {project_name}_expensive_scan_interval =
	CreateConVar("{project_name}_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local {project_name}_force_download =
	CreateConVar("{project_name}_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download {project_name} resources \z
...
if {project_name}_force_download:GetBool() then
	resource.AddWorkshop(workshopID)
end

util.AddNetworkString("{project_name}_nag")
util.AddNetworkString("{project_name}_navgen")
...
local VECTOR_{project_name}_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_{project_name}_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("{project_name}: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other {project_name}s, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "{project_name}"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "{project_name}MissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "{project_name}" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why {project_name} isn't working.
	net.Start("{project_name}_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("{project_name}: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("{project_name}: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("{project_name}: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("{project_name}: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("{project_name}: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, {project_name} in pairs(ents.FindByClass("{project_name}")) do
		{project_name}:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "{project_name}NavGen", navEndGenerate)
	else
		print("{project_name}: nav_generate failed to initialize")
...
concommand.Add("{project_name}_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("{project_name}: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. {project_name} doesn't use
	...
	elseif isConsole then
		print("{project_name}: Beginning nav_generate requested by server console")
	else
		return
	end
	...
	net.Start("{project_name}_navgen")
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = {project_name}_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `{project_name}_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if {project_name}_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = {project_name}_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. {project_name} means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not {project_name}_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = {project_name}_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = {project_name}_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = {project_name}_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = {project_name}_attack_distance:GetInt()
		...
		local repathInterval = {project_name}_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and {project_name}_allow_jump:GetBool()
		...
		local hidingScanInterval = {project_name}_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = {project_name}_hiding_repath_interval:GetFloat()
...
local MAT_{project_name} = Material("{project_name}/{project_name}")
killicon.Add("{project_name}", "{project_name}/killicon", color_white)
language.Add("{project_name}", "{project_name} ")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("{project_name}: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a {project_name}.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local {project_name}_music_volume =
	CreateConVar("{project_name}_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by {project_name}. (0-1, where 0 is muted)")

-- If another {project_name} comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, {project_name}s do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_{project_name}_PANIC_COUNT {project_name}s are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of {project_name}.
local MUSIC_{project_name}_PANIC_COUNT = 8

local MUSIC_{project_name}_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_{project_name}_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("{project_name}") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("{project_name}PanicMusicUpdate")
	...
	local userVolume = math.Clamp({project_name}_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "{project_name}" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_{project_name}_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear {project_name} taunt us.
...
local function startTimer()
	if not timer.Exists("{project_name}PanicMusicUpdate") then
		timer.Create("{project_name}PanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_{project_name})

	-- Get the normal vector from {project_name} to the player's eyes, and then compute
	...
	-- {project_name} should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("{project_name}HUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("{project_name}HUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("{project_name} is studying this map.", "{project_name}HUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "{project_name}HUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "{project_name}HUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "{project_name}HUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "{project_name}HUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("{project_name}_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "{project_name}NavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. {project_name} doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("{project_name}_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for {project_name} to figure \z
		this map out.\n\z
...
net.Receive("{project_name}_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! {project_name} doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet {project_name}-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! {project_name} doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to {project_name}.\n\z
			\n\z
			If you ARE the server host, you can run {project_name}_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not {project_name}-compatible!",
			"Ok", nil,
...
--
list.Set("NPC", "{project_name}", {
	Name = "{project_name}",
	Class = "{project_name}",
	Category = "Vovsss Nextbots",
	AdminOnly = true
})
