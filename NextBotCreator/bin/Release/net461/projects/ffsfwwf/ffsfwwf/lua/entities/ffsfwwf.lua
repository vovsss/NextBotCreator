...
ENT.JumpSound = Sound("ffsfwwf/jump.mp3")
ENT.JumpHighSound = Sound("ffsfwwf/spring.mp3")
ENT.TauntSounds = {
	Sound("ffsfwwf/pieceofcake.mp3"),
	Sound("ffsfwwf/stepitup.mp3"),
	Sound("ffsfwwf/tooeasy.mp3"),
	Sound("ffsfwwf/tooslow.mp3"),
}
local chaseMusic = Sound("ffsfwwf/panic.mp3")
...
local ffsfwwf_acquire_distance =
	CreateConVar("ffsfwwf_acquire_distance", 2500, FCVAR_NONE,
	"The maximum distance at which ffsfwwf will chase a target.")

local ffsfwwf_spawn_protect =
	CreateConVar("ffsfwwf_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, ffsfwwf will not target players or hide within 200 units of \z
	a spawn point.")

local ffsfwwf_attack_distance =
	CreateConVar("ffsfwwf_attack_distance", 80, FCVAR_NONE,
	"The reach of ffsfwwf's attack.")

local ffsfwwf_attack_interval =
	CreateConVar("ffsfwwf_attack_interval", 0.2, FCVAR_NONE,
	"The delay between ffsfwwf's attacks.")

local ffsfwwf_attack_force =
	CreateConVar("ffsfwwf_attack_force", 800, FCVAR_NONE,
	"The physical force of ffsfwwf's attack. Higher values throw things \z
	farther.")

local ffsfwwf_smash_props =
	CreateConVar("ffsfwwf_smash_props", 1, FCVAR_NONE,
	"If set to 1, ffsfwwf will punch through any props placed in their way.")

local ffsfwwf_allow_jump =
	CreateConVar("ffsfwwf_allow_jump", 1, FCVAR_NONE,
	"If set to 1, ffsfwwf will be able to jump.")

local ffsfwwf_hiding_scan_interval =
	CreateConVar("ffsfwwf_hiding_scan_interval", 3, FCVAR_NONE,
	"ffsfwwf will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant ffsfwwfs are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local ffsfwwf_hiding_repath_interval =
	CreateConVar("ffsfwwf_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to ffsfwwf's hiding spot will be redetermined every X seconds.")

local ffsfwwf_chase_repath_interval =
	CreateConVar("ffsfwwf_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of ffsfwwf's target will be redetermined every \z
	X seconds.")

local ffsfwwf_expensive_scan_interval =
	CreateConVar("ffsfwwf_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local ffsfwwf_force_download =
	CreateConVar("ffsfwwf_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download ffsfwwf resources \z
...
if ffsfwwf_force_download:GetBool() then
	resource.AddWorkshop(workshopID)
end

util.AddNetworkString("ffsfwwf_nag")
util.AddNetworkString("ffsfwwf_navgen")
...
local VECTOR_ffsfwwf_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_ffsfwwf_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("ffsfwwf: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other ffsfwwfs, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "ffsfwwfMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "ffsfwwf" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why ffsfwwf isn't working.
	net.Start("ffsfwwf_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("ffsfwwf: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("ffsfwwf: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("ffsfwwf: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("ffsfwwf: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("ffsfwwf: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, ffsfwwf in pairs(ents.FindByClass("ffsfwwf")) do
		ffsfwwf:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "ffsfwwfNavGen", navEndGenerate)
	else
		print("ffsfwwf: nav_generate failed to initialize")
...
concommand.Add("ffsfwwf_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("ffsfwwf: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. ffsfwwf doesn't use
	...
	elseif isConsole then
		print("ffsfwwf: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = ffsfwwf_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `ffsfwwf_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if ffsfwwf_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = ffsfwwf_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. ffsfwwf means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not ffsfwwf_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = ffsfwwf_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = ffsfwwf_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = ffsfwwf_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = ffsfwwf_attack_distance:GetInt()
		...
		local repathInterval = ffsfwwf_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and ffsfwwf_allow_jump:GetBool()
		...
		local hidingScanInterval = ffsfwwf_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = ffsfwwf_hiding_repath_interval:GetFloat()
...
local MAT_ffsfwwf = Material("ffsfwwf/ffsfwwf")
killicon.Add("ffsfwwf", "ffsfwwf/killicon", color_white)
language.Add("ffsfwwf", "ffsfwwf ")
...
local lastPanic = 0 -- The last time we were in music range of a ffsfwwf.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local ffsfwwf_music_volume =
	CreateConVar("ffsfwwf_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by ffsfwwf. (0-1, where 0 is muted)")

-- If another ffsfwwf comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, ffsfwwfs do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_ffsfwwf_PANIC_COUNT ffsfwwfs are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of ffsfwwf.
local MUSIC_ffsfwwf_PANIC_COUNT = 8

local MUSIC_ffsfwwf_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_ffsfwwf_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("ffsfwwf") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("ffsfwwfPanicMusicUpdate")
	...
	local userVolume = math.Clamp(ffsfwwf_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "ffsfwwf" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_ffsfwwf_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear ffsfwwf taunt us.
...
local function startTimer()
	if not timer.Exists("ffsfwwfPanicMusicUpdate") then
		timer.Create("ffsfwwfPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_ffsfwwf)

	-- Get the normal vector from ffsfwwf to the player's eyes, and then compute
	...
	-- ffsfwwf should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("ffsfwwfHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("ffsfwwfHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("ffsfwwf is studying this map.", "ffsfwwfHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "ffsfwwfHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "ffsfwwfHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "ffsfwwfHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "ffsfwwfHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("ffsfwwf_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "ffsfwwfNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. ffsfwwf doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("ffsfwwf_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for ffsfwwf to figure \z
		this map out.\n\z
...
net.Receive("ffsfwwf_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! ffsfwwf doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet ffsfwwf-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! ffsfwwf doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to ffsfwwf.\n\z
			\n\z
			If you ARE the server host, you can run ffsfwwf_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not ffsfwwf-compatible!",
			"Ok", nil,
